...
import ResettableTextField from 'ra-ui-materialui/lib/input/ResettableTextField'
...
export class SelectInput extends Component {
  ...
  constructor(props) {
    ...
    const { input, optionId, dataStructure } = props

    let value

    if (dataStructure === DataStructures.RECORD) {
      value = get(input.value, optionId, '')
    } else {
      value = input.value
    }
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.input.value !== this.props.input.value) {
      const { optionId, dataStructure } = this.props

      let nextValue

      if (dataStructure === DataStructures.RECORD) {
        nextValue = get(nextProps.input.value, optionId)
      } else {
        nextValue = nextProps.input.value
      }
  ...
  handleChange = eventOrValue => {
    ...
    const { dataStructure, optionId, choices } = this.props

    let inputValue

    if (dataStructure === DataStructures.RECORD) {
      inputValue = choices.find(item => get(item, optionId) === value)
    } else {
      inputValue = value
    }
    ...
    this.props.input.onBlur(inputValue)
    this.setState({ value })
  }
  ...
  renderMenuItem = choice => {
    const { optionValue, optionId, disableValue, dataStructure } = this.props

    let value, key

    if (dataStructure === DataStructures.RECORD) {
      key = value = get(choice, optionId)
    } else {
      key = value = get(choice, optionValue)
    }

    return (
      <MenuItem key={key} value={value} disabled={get(choice, disableValue)}>
