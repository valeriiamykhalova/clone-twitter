...
export const getSelectedReferencesStatus = (input, referenceRecords) => {
  if (
    ...
    return REFERENCES_STATUS_READY
  }

  // "array" data type || "booleanMap" and "numericMap" data types
  const valueRecordsLength =
    input.value.length || Object.keys(input.value).length

  // all records from reference are fetched
  if (valueRecordsLength === referenceRecords.length) {
    return REFERENCES_STATUS_READY
  }

  // some of records were not fetched
  if (referenceRecords.length > 0) {
    return REFERENCES_STATUS_INCOMPLETE
  }

  return REFERENCES_STATUS_EMPTY
}
...
export const getStatusForArrayInput = ({
  ...
  return {
    ...
    error:
      matchingReferencesError &&
      (!input.value ||
        (input.value && selectedReferencesData === REFERENCES_STATUS_EMPTY))
